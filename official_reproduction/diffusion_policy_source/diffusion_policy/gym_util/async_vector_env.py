...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset(self, *args, **kwargs):
        """Gym 0.26 compatibility wrapper with the old Gym return contract."""
        self.reset_async()
    ...
    def reset_wait(self, timeout=None):
        ...
        self._raise_if_errors(successes)
        self._state = AsyncState.DEFAULT
        # Gym 0.26 wrappers may return (observation, info). The upstream
        # Diffusion Policy runners use the old Gym API and expect only the
        # observation array.
        results = tuple(x[0] if isinstance(x, tuple) and len(x) == 2 else x for x in results)
